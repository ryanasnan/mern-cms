...
export const logout = (history) => async(dispatch) => {
	// if before logout the user still on private route (exp: mystory), it will be redirect on authorize process, so it will better to directly push on landing page
	await history.push('/');
	await dispatch({ type: LOGOUT });
	await dispatch(setAlert('You have been logout, Have a Good Day!!!', 'info'));
};